...
args <- commandArgs(T) # returns the command line arguments that were passed to the R script or R session.
...
my.count1 <- read.table(args[1],h=F) # args[1]: This is a variable that refers to the first command line argument passed to the R script. 
...
# modify this trunk for my use
inds <- as.numeric(length(cells.keep))
...
saveRDS(data.sparse,file=args[2])
